...
def load_data(path: str) -> tuple:
    ...
    drop = ['t0(min)45', 'tr(min)45', 't0(min)5', 'tr(min)5']
    
    # Drop unnecessary columns and create a list of molecules
    molecules = df_mydata['Compound_name']
    df_mydata = df_mydata.drop(['Compound_name', 'SMILES', 'Formula', 'MW'], axis=1)
    df_mydata = df_mydata.drop(drop, axis=1)
    
    # Left join the two dataframes based on the column 'SMILES'
    #df = pd.merge(df_mydata, df_descriptors, how='left', on='SMILES')
